...
export function findScheduleConflicts(target: SectionRow, against: SectionRow[]): SectionRow[] {
  return against.filter((c) => c.timeSlots.some((a) => target.timeSlots.some((b) => overlaps(a, b))));
}
